...
1. clicking an open-ended condition occurrence in the D3 timeline does not crash
...
1. `observeEvent(input$bar_end)` in [patientsDesigner.R](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/R/patientsDesigner.R#L492)
2. `cdm[[type]]$updateDates(...)` in [patientsDesigner.R](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/R/patientsDesigner.R#L500)
3. `data.table::set(...)` in [cdmConstructor.R](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/R/cdmConstructor.R#L195)
...
If `end_date` arrives as `character(0)`, `NULL`, or an omitted browser value that becomes zero-length after coercion:
...
1. not `NULL`
2. length `0`
3. invalid as the RHS for `data.table::set()` on an existing column

So the specific crash is a zero-length optional-date assignment bug.

## Important clarification about the interaction

The current error is raised from `input$bar_end`, which is emitted at the end of a drag interaction, not from a plain click.

In [cdm_timeline.js](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/inst/d3/cdm_timeline.js):
...
That matters because any visual fallback that gives an open-ended row a fake width will also make whole-bar dragging implicitly create a real `end_date`.

That is not the desired UX. The requirement is:

1. selecting the row must not crash
2. adding the first end date must be explicit, not a side effect of moving the row

## Evidence in the fixture

The file [mod_test_file_updated.json](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/tests/testthat/testCases/mod_test_file_updated.json) contains multiple open-ended conditions, for example:
...
In [cdmConstructor.R](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/R/cdmConstructor.R#L179):
...
If we render open-ended rows as short pseudo-bars, the current drag handlers would let users:

1. move the row
2. left-drag the row
3. accidentally materialize a real end date

That would violate the intended behavior that the first end date is added only when the user wants it.
...
This avoids the earlier mistake of silently converting a broken `start_date` into `NA`.

### Step 2. Sanitize the `bar_end` payload at the observer boundary

The crash is happening at the D3-to-server boundary, so that boundary should be normalized explicitly.

Instead of letting `observeEvent(input$bar_end)` call `updateDates()` with raw browser values, convert the payload first.
...
This is the most important fix because it directly addresses the observed stack trace.
...
`updateDates()` must only ever assign a length-1 `Date`, including `as.Date(NA)` for the optional end date.
...
Do not persist a fake end date into the CDM table. Instead, derive display-only fields.
...
If an open-ended row is drawn with a short visual fallback, then the current handlers would make these actions implicitly create an end date:
...
4. right-handle drag: optional, and if enabled it explicitly creates the first end date
...
For open-ended rows, the right-side interaction can be one of two designs:

1. manual only: user sets `condition_end_date` in the form
2. manual or right-drag: right-drag creates the first end date explicitly

Either is acceptable, but the proposal should choose one explicitly. The safer initial fix is manual entry only.
...
That separation reduces the chance that a simple selection path triggers date mutation logic.
...
To make the boundary testable, extract the coercion into a pure helper like:
...
1. Reproduce with [mod_test_file_updated.json](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/tests/testthat/testCases/mod_test_file_updated.json).
2. Add `normalizeRequiredDate()`, `normalizeOptionalDate()`, and `normalizeBarEndPayload()`.
...
8. Decide whether the first end date is added manually only or also via right-drag.
9. Add regression tests for the observer boundary and the open-ended workflow.
...
1. load [mod_test_file_updated.json](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/tests/testthat/testCases/mod_test_file_updated.json)
...
1. Start `patientDesigner()`.
2. Load [mod_test_file_updated.json](/Users/cbarboza/Documents/mi-erasmusmc/PatientGenerator/tests/testthat/testCases/mod_test_file_updated.json).
...
1. open-ended `condition_occurrence` rows from JSON fixtures remain valid
2. selecting them in D3 no longer crashes the app
3. `updateDates()` no longer receives a zero-length RHS for `condition_end_date`
4. a broken `start_date` is rejected instead of silently overwritten with `NA`
