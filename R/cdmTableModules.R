cdmTableUI <- function(id) {
  ...
  card(
    full_screen = TRUE,
    card_body(
      fluidRow(column(2, actionButton(ns("add"), "Add")),
               # column(2, actionButton(ns("duplicate"), "Duplicate")),
               column(2,conceptSearchUI(ns("concept_search"), buttonLabel = "Search Concept")),
...
cdmTableServer <- function(id, cdm, person_id_selected, syncing) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    conceptSearchServer(
      ...
      onConceptSelected = function(conceptId) {
        shiny::updateTextInput(session, table_concept_id, value = as.character(conceptId))
      },
      placeholderText = "e.g. Metformin"
    )

    # Observe the event id to update its corresponding fields in the interface
    observeEvent(input[[table_event_id]], {
      req(input[[table_event_id]])

      # Retrieve row
      cdmTable <- cdm[[id]]$data()
      cdmTableRow <- cdmTable |>
        filter(.data[[table_event_id]] == input[[table_event_id]] |> as.integer())
    ...
    observe({
      ...
      no_date_inputs <- table_inputs[grep("date", names(table_inputs), invert = TRUE)]
      non_empty_inputs <- any(vapply(no_date_inputs, function(x) {
        is.character(x) && nzchar(x)
      }, logical(1)))
      req(non_empty_inputs)
      cdm_table_row <- cdm[[id]]$extractRow(event_id = input[[table_event_id]])
      new_input_data <- as.data.frame(no_date_inputs)
      cdm_table_row[is.na(cdm_table_row)] <- ""
      new_input_data[is.na(new_input_data)] <- ""
      req(any(cdm_table_row[,  names(no_date_inputs)] != new_input_data, na.rm = TRUE))
      args <- c(list(event_id = as.integer(input[[table_event_id]])), no_date_inputs)
      # browser()
    ...
    observeEvent(input$delete, {
      ...
      if (tableDeleted[[table_event_id]] %>% length() == 0) {
        ...
        choicesPerson <- tableDeleted[[table_event_id]][tableDeleted$person_id == person_id_selected()]
        updateSelectInput(session,
                          table_event_id,
    ...
    # # Elongation
    observeEvent(
      list(input[[table_start_date]],
           input[[table_end_date]]),
      {
        print(syncing)
        req(!syncing())
