test_that("Chaining LLM thought for synthetic patient generation", {
  ...
  expect_error({
    patientGenerator <- patientChat$new(model = "claude", provider = "anthropic")
  })
  expect_no_error({
    patientGenerator <- patientChat$new(model = "claude-opus-5", provider = "anthropic")
    patientGenerator <- patientChat$new(model = "claude-sonnet-5", provider = "anthropic")
  })
...
test_that("patientChat API flow creates loadable test set", {
  ...
  expect_error(patientChat$new(model = "claude", provider = "anthropic"))

  generator <- patientChat$new(model = model, echo = "none", provider = "anthropic")
...
test_that("patientChat with local codelist returns retrievable concepts", {
  ...
  generator <- patientChat$new(model = model, codelist_data = codelist_data, echo = "none", provider = "anthropic")
