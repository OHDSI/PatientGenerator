...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/PatientGenerator)](https://CRAN.R-project.org/package=PatientGenerator)
[![R CMD check](https://github.com/OHDSI/PatientGenerator/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/OHDSI/PatientGenerator/actions/workflows/R-CMD-check.yaml)
...
# Pre-computed output for documentation preview
library(PatientGenerator)
library(dplyr)

test_file <- system.file("testCases", "test_diabetes_patients.json", package = "PatientGenerator")
if (test_file == "") {
  test_file <- "tests/testthat/testCases/test_diabetes_patients.json"
}

cdm_obj <- PatientGenerator:::cdmConstructor$new()
cdm_obj$loadJsonTestSet(test_file)

person_df <- cdm_obj$person$data() |> 
  select(person_id, gender_concept_id, year_of_birth, person_source_value) |> 
  head(5)

cat("cdm$person |> collect() |> head(5)\n")
print(person_df)
```
...
# Pre-computed output for iterative refinement example
cat("cdm$person |> collect() |> head(5)\n")
cdm_obj$person$data() |> 
